...
void LCD_ShowPicture(uint16_t usXstar,uint16_t usYstar,uint16_t usPicH,uint16_t usPicV,unsigned short *pic)
{	
	u16 m,h;
	u16 *data=(u16*)pic; 
	for(h=0+usYstar;h<usPicV;h++)
	{
		for(m=0+usXstar;m<usPicH;m++)
		{
